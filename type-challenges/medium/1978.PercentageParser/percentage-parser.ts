...
type Prefix<A extends string> = A extends `${infer L}${infer R}` ? L extends '+' | '-' ? L : '' : ''
type Suffix<A extends string> = A extends `${infer L}%` ? [L, '%'] : [A, '']
type PercentageParser<A extends string> = [Prefix<A>, ...(Prefix<A> extends '' ? Suffix<A> : A extends `${'+' | '-'}${infer R}` ? Suffix<R> : never)]
