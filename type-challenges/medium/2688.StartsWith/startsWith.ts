...
type StartsWith<T extends string, U extends string> = U extends '' ? true : T extends `${infer A}${infer B}` ? U extends `${infer C}${infer D}` ? A extends C ? StartsWith<B, D> : false : never : false
